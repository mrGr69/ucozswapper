...
.topbar{display:flex;align-items:center;justify-content:space-between;padding:22px 0;font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.brand{display:flex;align-items:center;gap:10px}.brand-mark{display:grid;width:32px;height:32px;place-items:center;background:var(--ink);color:#f8fafc}.source{color:var(--muted)}
.hero{display:grid;grid-template-columns:minmax(0,.96fr) minmax(0,1.04fr);gap:clamp(28px,6vw,88px);align-items:center;min-height:680px;padding:42px 0 76px}.hero-media{position:relative;min-height:580px;border-radius:5px;background:linear-gradient(145deg,var(--accent-soft),rgba(255,255,255,.42));overflow:hidden}.hero-media>img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain}.hero-copy{padding:24px 0}.layout-media-right .hero-media{order:2}.layout-media-right .hero-copy{order:1}.eyebrow{margin:0;color:var(--accent);font-size:12px;font-weight:900;letter-spacing:.16em;text-transform:uppercase}.hero h1{max-width:760px;margin:18px 0 20px;font-size:clamp(44px,6.2vw,86px);font-weight:900;letter-spacing:-.065em;line-height:.92}.lead{max-width:650px;margin:0;color:var(--muted);font-size:clamp(17px,2vw,21px);line-height:1.55}.price{display:block;margin-top:30px;font-size:clamp(30px,4vw,48px);font-weight:900;letter-spacing:-.04em}.actions{display:flex;flex-wrap:wrap;align-items:center;gap:14px;margin-top:24px}.cta{display:inline-flex;align-items:center;gap:12px;padding:17px 22px;border-radius:5px;background:var(--accent);color:#fff;text-decoration:none;font-weight:900;box-shadow:0 16px 40px var(--accent-soft);transition:transform .2s ease,filter .2s ease}.cta:hover{transform:translateY(-3px);filter:brightness(.94)}.cta-note{max-width:250px;color:var(--muted);font-size:12px;line-height:1.45}
...
.preset-editorial .hero{grid-template-columns:minmax(0,.82fr) minmax(0,1.18fr)}.preset-editorial .hero-media{min-height:650px}.preset-editorial .hero h1{font-family:Georgia,serif;font-weight:700;letter-spacing:-.05em}.preset-editorial .benefits{grid-template-columns:repeat(2,minmax(0,1fr))}.preset-spec-driven .hero{min-height:600px}.preset-spec-driven .hero-media{min-height:500px}.preset-spec-driven .content-grid{grid-template-columns:minmax(0,.72fr) minmax(0,1.28fr)}
:is(.preset-red_dark,.preset-green_dark,.preset-toxic,.preset-midnight) .hero-media{background:linear-gradient(145deg,var(--accent-soft),var(--panel));box-shadow:inset 0 0 0 1px var(--line),0 30px 90px rgba(0,0,0,.35)}
...
.preset-green_dark .hero-media{background:radial-gradient(circle at 50% 42%,rgba(33,217,139,.22),transparent 55%),var(--panel)}.preset-green_dark .section h2{color:#d7ffea}
...
<header class="topbar"><div class="brand"><span class="brand-mark">US</span><span>UcozSwapper</span></div><span class="source">${escapeHtml(product.platform)} · ${escapeHtml(product.productId)}</span></header>
<section class="hero">
...
<section class="section content-grid"><article class="story"><p class="section-kicker">О товаре</p><h2>${escapeHtml(product.title)}</h2><p>${escapeHtml(product.description || content.hero.subheadline)}</p></article>${specifications ? `<article class="spec-wrap"><p class="section-kicker">Детали</p><h2>Характеристики</h2><dl class="specs">${specifications}</dl></article>` : ""}</section>
